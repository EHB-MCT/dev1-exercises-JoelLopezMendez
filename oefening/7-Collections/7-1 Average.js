...
function setup() {
    for (let i = 0; i < 100; i++) {
        x[i] = Utils.randomNumber(0, wdth);
        y[i] = Utils.randomNumber(0, height);
        size[i] = Utils.randomNumber(5, 50);
        numero[i] = Utils.randomNumber(0, x.length - 1);
    }
}
...
function update() {
    ...
    for (let i = 0; i < x.length; i++) {
        drawSnail(x[i], y[i], size[i], numero[i]);
